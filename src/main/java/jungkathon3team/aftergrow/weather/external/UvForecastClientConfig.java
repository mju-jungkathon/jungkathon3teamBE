 ...
 * 전자는 <b>빈 문자열도 "설정됨"으로 취급</b>하고(기본값 {@code ${KMA_AUTH_KEY:}}가 정확히 그 경우다),
...
public class UvForecastClientConfig {
    ...
    public UvForecastClient uvForecastClient(RestClient.Builder builder,
                                             ...
                                             @Value("${kma.auth-key:}") String authKey) {
        if (authKey == null || authKey.isBlank()) {
            log.warn("kma.auth-key가 설정되지 않아 UV 예보를 모의값으로 응답합니다. "
                    + "실제 데이터가 필요하면 application-local.yml(로컬) 또는 .env(배포)에 키를 넣으세요.");
            return new MockUvForecastClient();
        }
        return new KmaUvForecastClient(builder, objectMapper, authKey);
    }
}
